...
- **右摇杆 → 鼠标移动**，R3/L3 → 鼠标左/右键（SendInput 鼠标注入）
- 多配置档管理，Start 键循环切换
...
1. **右摇杆**：移动鼠标（推得越远移动越快，死区 0.15 防漂移）
2. **R3**（右摇杆下按）：鼠标左键（点击/选中）
3. **L3**（左摇杆下按）：鼠标右键（右键菜单）

> 速度与死区可在 `config.json` 调整：`mouse_speed`（满偏移每帧像素，默认 8）、`mouse_deadzone`（默认 0.15）。
